...
def get_depth_vd(depth_data: pd.DataFrame) -> VideoData:
    ...
    for img_data in depth_data['data']:
        ...
        depth_video.frames.append(depth_transformed)
...
def save_video(filename: str, video: VideoData, fps: int = 15, codec: str = 'mp4v', colored: bool = True) \
    ...
    writer = skvideo.io.FFmpegWriter(filename, inputdict={'-r': str(fps), '-s':'{}x{}'.format(video.width,video.height)},
            outputdict={'-r': str(fps), '-c:v': 'libx264', '-crf': str(crf), '-preset': 'ultrafast', '-pix_fmt': 'yuv420p'})
...
if __name__ == '__main__':
    ...
    # save_video(filename='rgb_video11.mp4', video=rgb_vd)
    save_video(filename='depth_video11.mp4', video=depth_vd)
